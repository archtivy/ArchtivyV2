...
export async function GET(req: NextRequest) {
  ...
  const neverUsed = searchParams.get("neverUsed")?.trim() ?? "";
  ...
  let query = supabase
    .from("listings")
    .select(
      "id,title,description,location,year,created_at,updated_at,cover_image_url,category,product_type,material_or_finish",
      { count: "exact" }
    )
    .eq("type", "product")
    .is("deleted_at", null)
  ...
  if (missing === "1") query = query.or("description.is.null,cover_image_url.is.null");
  ...
  const [imagesRes, linksRes] = await Promise.all([
    ...
    ids.length ? supabase.from("project_product_links").select("product_id").in("product_id", ids) : { data: [] },
  ]);
  ...
  const rows = (data ?? [])
    .map((r: Record<string, unknown>) => ({
      ...
      used_in_projects: linkCount[r.id as string] ?? 0,
      word_count: r.description
        ? String(r.description).split(/\s+/).filter(Boolean).length
        : 0,
    }))
    .filter((r) => (neverUsed === "1" ? r.used_in_projects === 0 : true));
