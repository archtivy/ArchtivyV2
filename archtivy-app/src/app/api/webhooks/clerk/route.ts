...
export async function POST(req: Request) {
  ...
  let evt: { type: string; data?: { email_addresses?: { email_address: string }[] }; data?: { primary_email_address_id?: string } };
  try {
    evt = wh.verify(body, { "svix-id": svixId, "svix-timestamp": svixTimestamp, "svix-signature": svixSignature }) as typeof evt;
  } catch (err) {
