...
function IconFollowing({ className }: { className?: string }) {
  return (
    <svg className={className} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
...
export function TopNavAuth({ displayName, role, locationCity, featureListingEnabled }: TopNavAuthProps) {
  const { user } = useUser();
          ...
          {open && (
            <DropdownPanel>
              ...
              <div role="none" className="px-1.5 py-1.5">
                ...
                <Link role="menuitem" href="/me/following" className={itemCls} onClick={close}>
                  <IconFollowing className={iconCls} />
                  Following
                </Link>
