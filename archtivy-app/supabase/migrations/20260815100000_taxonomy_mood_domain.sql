...
--
-- ── IDEMPOTENCY ─────────────────────────────────────────────────────────────
-- Per-row WHERE NOT EXISTS on (domain, slug), NOT `on conflict do nothing`.
--
-- An earlier draft used the latter. That was wrong: with no conflict target it
-- only fires on a unique-index violation, and nothing here is uniquely indexed
-- — so a second run would have inserted ten duplicate mood terms while the
-- comment claimed re-runs were safe.
--
-- A unique index on (domain, slug) would also make this correct, but it is not
-- worth the risk: 140 slug values already repeat across the table, and adding a
-- constraint would fail — or silently demand a cleanup — if any of those
-- repeats sit inside a single domain. This migration should not be the thing
-- that discovers that.
--
-- Each VALUES row is tested independently, so a partially applied state
-- completes correctly instead of erroring or double-inserting.
...
select
  ...
  true
from (values
  ('mood'::text, 'calm'::text,       'calm'::text,       'Calm'::text,       1),
  ('mood',       'cozy',             'cozy',             'Cozy',             2),
  ('mood',       'dark',             'dark',             'Dark',             3),
  ('mood',       'bright',           'bright',           'Bright',           4),
  ('mood',       'elegant',          'elegant',          'Elegant',          5),
  ('mood',       'organic',          'organic',          'Organic',          6),
  ('mood',       'luxury',           'luxury',           'Luxury',           7),
  ('mood',       'industrial',       'industrial',       'Industrial',       8),
  ('mood',       'minimal',          'minimal',          'Minimal',          9),
  ('mood',       'warm',             'warm',             'Warm',             10)
) as v(domain, slug, slug_path, label, sort_order)
where not exists (
  -- Scoped by domain, so the existing STYLE node 'industrial' does not block
  -- the MOOD node of the same slug. That is the intended outcome — see below.
  select 1
...
-- Assert the end state rather than trusting the insert. This holds on a first
-- run and on every re-run; if it ever fails, the transaction rolls back and
-- leaves the constraint widening in place only if the whole migration commits.
do $$
declare
  v_count integer;
begin
  select count(*) into v_count from public.taxonomy_nodes where domain = 'mood';
  if v_count <> 10 then
    raise exception 'Expected exactly 10 mood terms after seeding, found %. Aborting.', v_count;
  end if;
end $$;
